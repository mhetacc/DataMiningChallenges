...
All that follows is based on the datasets provided in the challenge page (\textit{rice\_test.csv} and \textit{rice\_train.csv}), even thought they both stem from an original one, which can be found at the following link: \url{https://www.muratkoklu.com/datasets/}.
...
I used the \textit{glmnet} library, which takes care of the variable standardization for us and has a built-in cross-validation functionionality to compute the best possible $\lambda$ (code \ref{code:rice_ridge}). The resulting model was slightly worse than linear regression, since $MSE = 0.07924703$. That being said, we can see in plot \ref{fig:rice_ridge} an effective shrinkage of most coefficients towards zero, with the exception of \textit{Eccentricity}, which we already recognized as not very impactful on the overall variance and prediction performances.
...
The last method I tried is \textit{random forest} with Leave-One-Out-Cross-Validation, using the \textit{caret} library still. Since this method is extremely computationally expensive, I had to leverage the library \textit{doParallel} to use all available CPU cores, and I used the faster implementation \textit{ranger}. I also reduced the amount of predictors by combining the highly correlated ones (we already saw that dropping them yield similar to worse results). The code can be seen at listing \ref{code:rice_randomforest}.

The optimal model, with $mtry = 2$, $splitrule = extratress$ and $min.node.size = 5$, yielded a slighly worse result than KNN trained on all features: $MSE = 0.0589397$. The plot for the \textit{RMSE} can be seen in figure \ref{fig:rice_randomforest}
