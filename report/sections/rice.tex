...
All that follows is based on the datasets provided in the challenge page (\textit{rice\_test.csv} and \textit{rice\_train.csv}), even though they both stem from an original one, which can be found at the following link: \url{https://www.muratkoklu.com/datasets/}.

\subsubsection{Scatter Plot}

From the \textit{scatter plot} (figure \ref{fig:scatterplot_rice_loess}) we can infer that there are four extremely correlated features: \textit{Area}, \textit{Perimeter}, \textit{Convex\_Area} and \textit{Major\_Axis\_Length}.
...
To get a better idea of the importance of each feature, I trained a simple linear regression model, and looked at the result with \textit{summary(fit)}, which can be seen in table \ref{tab:regression_coefficients}. The significance stars tell us visually that, with the exception of \textit{Minor\_Axis\_Length}, highly correlated features contribute strongly to the model, while \textit{Eccentricity} and \textit{Extent} contribute very little.
...
A good way to aggregate and simplify data is by decomposing it into its principal components (centered in zero and scaled to have unit variance). Components' summary can be seen in table \ref{tab:rice_pca_importance}, and it shows us that more than 99\% of overall variance can be explained with just three components, so a 2D visualization with only the first two components should give us a good idea of the whole dataset (figure \ref{fig:rice_2pca_plot}). The plot suggests that data is well separated, with similar classes clustering nicely. This should make classification easier. 
...
First I split the training dataset into training and validation sets, fitted the model and computed its mean square error: $MSE = 0.07354557$. Since we know from the preliminary observations that some features are highly correlated (so the dataset suffers from collinearity), I tried to transform the dataset to reduce it. A straightforward approach is to compute the average of all of them (listing \ref{code:avg_rice_correlated}), but this yielded a slightly worse result: $MSE \approx 0.074$.

Another alternative would be to drop the features altogether, after measuring their variance inflation factor. As a rule of thumb, a \textit{VIF} value above than indicates a problematic amount of collinearity, so I tried to drop features iteratively to see if the model improves, as shown in table \ref{tab:vif_drop_analysis}. For each dropped variable I computed the \textit{VIFs} and the \textit{MSE} of the resulting model. As we can see, dropping \textit{Area} resulted in performances similar to combining the variables, while dropping the other features worsened prediction performances even further.
...
I used the \textit{glmnet} library, which takes care of the variable standardization for us and has a built-in cross-validation functionality to compute the best possible $\lambda$ (code \ref{code:rice_ridge}). The resulting model was slightly worse than linear regression, since $MSE = 0.07924703$. That being said, we can see in plot \ref{fig:rice_ridge} an effective shrinkage of most coefficients towards zero, with the exception of \textit{Eccentricity}, which we already recognized as not very impactful on the overall variance and prediction performances.
...
Very similar to ridge regression, except \textit{glmnet} gets called with \textit{alpha = 1}. We get a slightly better result than ridge, with $MSE = 0.07348723$. From the plot (in figure \ref{fig:rice_lasso}), we can see that once again \textit{Eccentricity} seems to resist shrinkage, but overall coefficient reduction is much more accentuated compared to ridge, since lasso can set them to zero. 
...
I fitted the model with \textit{pls} library's built-in cross-validation, as seen in code \ref{code:rice_pcr}. The lowest error (figure \ref{fig:rice_pcr}) is achieved with seven components: $MSE =  0.07354557$, slightly worse than lasso regression.
...
I then tried some non-parametric methods, starting with \textit{LOESS} regression. This method allows up to four predictors, so I used the most useful ones previously identified: \textit{Area}, \textit{Perimeter}, \textit{Convex\_Area} and \textit{Major\_Axis\_Length}. Predictors are normalized by default. Then, I fitted three models with three different values for \textit{span}. Predictions must be sanitized since some test points can fall outside the local neighborhood (meaning being \textit{NA}). With $span = 0.1$ we have five such predictions. The code can be seen at \ref{code:rice_loess}. The results were disastrous ($MSE \approx 46$), so I tried to include only two predictors. This yielded the best result yet: with $span = 0.1$, $MSE=0.06099749$. All results are summarized in table \ref{tab:rice_loess}
...
I used the \textit{caret} library to fit \textit{KNN} both for classification and regression. The latter to get a comparable prediction with the other models, the former to be more logically sound, since \textit{Class} is a categorical target. The code for classification can be seen at \ref{code:rice_knn}, between the two methods the only difference is the \textit{metric} parameter: \textit{"Accuracy"} for classification, \textit{"RMSE"} for regression. For both methods I used Leave-One-Out Cross-Validation, and both RMSE plots can be seen in figure \ref{fig:rice_knn}. 
...
\begin{lstlisting}[language={R},label={code:rice_knn}, caption={KNN regression on rice dataset with Leave-One-Out Cross-Validation}]
train.control <- trainControl(method  = "LOOCV")
train$Class <- as.factor(train$Class) # necessary for classification 
...
\begin{figure}[h]
    ...
    \begin{subfigure}{0.4\textwidth}
        \includegraphics[width=\linewidth]{imgs/knn_regression.png}
        \caption{RMSE of KNN for regression}
    \end{subfigure}
    \hspace{.05\linewidth}
    \begin{subfigure}{0.4\textwidth}
        \includegraphics[width=\linewidth]{imgs/knn_classification.png}
        \caption{Accuracy of KNN for classification}
    \end{subfigure}

    \caption{RMSE and Accuracy respectively of KNN for regression and for classification with \textit{LOOCV} on \textit{rice\_train} dataset.}
    \Description{Graphs with error rate and precision for KNN models with different number of neighbors.}
    \label{fig:rice_knn}
\end{figure}

\subsubsection{Random Forest}
\label{sec:rice_randomforest}

The last method I tried is \textit{random forest} with Leave-One-Out Cross-Validation, using the \textit{caret} library still. Since this method is extremely computationally expensive, I had to leverage the library \textit{doParallel} to use all available CPU cores, and I used the faster implementation \textit{ranger}. I also reduced the amount of predictors by combining the highly correlated ones (we already saw that dropping them yielded similar or worse results). The code can be seen at listing \ref{code:rice_randomforest}.

The optimal model, with $mtry = 2$, $splitrule = extratrees$ and $min.node.size = 5$, yielded a slightly worse result than KNN trained on all features: $MSE = 0.0589397$. The plot for the \textit{RMSE} can be seen in figure \ref{fig:rice_randomforest}

\begin{lstlisting}[language={R},label={code:rice_randomforest}, caption={Random forest regression on \textit{rice\_train} dataset with Leave-One-Out Cross-Validation}]
train.control <- trainControl(method  = "LOOCV")
...
\begin{figure}[h]
  \centering
  \includegraphics[width=.7\linewidth]{imgs/randomforest.png}
  \caption{RMSE error for random forest on \textit{rice\_train} dataset.}
    \Description{Plot shows MSE getting lower as components are added.}
  \label{fig:rice_randomforest}
\end{figure}

\subsection{Rice Varieties Conclusions}
...
I decided not to try random forest with all predictors for time constraints: it required a 10-Cores machine (i7-13620H @ 4.90 GHz) twenty-five minutes to fit the model presented in section \ref{sec:rice_randomforest}.
